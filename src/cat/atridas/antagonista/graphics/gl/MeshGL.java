...
public class MeshGL extends Mesh {
  ...
  protected boolean loadBuffers(ByteBuffer _vertexBuffer, ArrayList<ByteBuffer> _submeshes, boolean _animated) {
    ...
    indexBuffers = new int[_submeshes.size()];
    for(int i = 0; i < indexBuffers.length; ++i) {
      indexBuffers[i] = glGenBuffers();
      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBuffers[i]);
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, _submeshes.get(i), GL_STATIC_DRAW);
    }
  ...
  protected void loadDefault() {
    LOGGER.severe("Not yet implemented, but I don't want to crash");
  }
