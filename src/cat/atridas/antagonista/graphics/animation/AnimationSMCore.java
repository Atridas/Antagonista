...
public class AnimationSMCore extends Resource {
  private static Logger LOGGER = Logger.getLogger(AnimationSMCore.class.getCanonicalName());
  
  /*
  private final HashMap<HashedString, AnimationInstance> basicLayerAnimationStates = new HashMap<>();
  private HashedString      basicLayerActiveStateID;
  private AnimationInstance basicLayerActiveState;
  private final HashMap<HashedString, Float> basicLayerFadingAnimationWeights = new HashMap<>();
  ...
  */
  ...
  public boolean load(InputStream is, HashedString extension) {
    try {
      ...
      Element basicLayerXML = (Element)(animationSMXML.getElementsByTagName("basic_layer").item(0));
      ...
      NodeList animationStatesXML = animationSMXML.getElementsByTagName("animation_state");
      for(int i = 0; i < animationStatesXML.getLength(); ++i) {
        ...
        NodeList animationXML = animationStateXML.getElementsByTagName("animation");
        
        assert animationXML.getLength() == 1;
        
        CoreAnimationNode animation = getAnimation((Element) animationXML.item(0));
        
        CoreAnimationState animationState = new CoreAnimationState(fadeIn, fadeOut, animation);
  ...
  private CoreAnimationNode getAnimation(Element animationXML) {
    ...
    if(type.equals("single animation")) {
      ...
      CoreAnimationNode animation1 = getAnimation((Element) animationsXML.item(0));
      CoreAnimationNode animation2 = getAnimation((Element) animationsXML.item(1));
      
      return new CoreAnimationNode(animation1, animation2, new HashedString(parameter));
      ...
      CoreAnimationNode baseAnimation = getAnimation((Element) animationsXML.item(0));
      
      return new CoreAnimationNode(new HashedString(animationId), baseAnimation, new HashedString(parameter));
    } else {
  ...
  final static class CoreAnimationState {
    ...
    CoreAnimationState(float _fadeIn, float _fadeOut, CoreAnimationNode _animationNode) {
      fadeIn = _fadeIn;
      fadeOut = _fadeOut;
  ...
  final static class CoreAnimationNode {
    ...
    // single animation
    CoreAnimationNode(CoreAnimationNode _animation1, CoreAnimationNode _animation2, HashedString parameter) {
    ...
    // additive animation
    CoreAnimationNode(HashedString _animationID, CoreAnimationNode base, HashedString parameter) {
