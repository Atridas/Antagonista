...
public class TechniquePassGL2 extends TechniquePassGL {
  private static Logger LOGGER = Logger.getLogger(TechniquePassGL2.class
      .getCanonicalName());
  ...
  private int modelViewProjectionUniform,
      modelViewUniform,
      modelViewITUniform,// , bonesUniform;
      bonePalete, bonePaleteIT, specialColor0, specialColor1, specialColor2,
      specialColor3, ambientUniform, directionalDirUniform,
      directionalColorUniform, specularFactorUniform,
      specularGlossinessUniform, heightUniform;
  ...
  protected void bindAttributes(int program) {
    ...
    if (tangents) {
      ...
      glBindAttribLocation(program, BITANGENT_ATTRIBUTE,
    ...
    if (bones) {
      glBindAttribLocation(program, BLEND_INDEX_ATTRIBUTE,
          BLEND_INDEX_ATTRIBUTE_NAME);
      glBindAttribLocation(program, BLEND_WEIGHT_ATTRIBUTE,
          BLEND_WEIGHT_ATTRIBUTE_NAME);
    }
    if (fontTechnique) {
      glBindAttribLocation(program, FONT_POSITION_ATTRIBUTE,
          FONT_POSITION_ATTRIBUTE_NAME);
      glBindAttribLocation(program, FONT_TEX_ATTRIBUTE, FONT_TEX_ATTRIBUTE_NAME);
      glBindAttribLocation(program, FONT_CHANNEL_ATTRIBUTE,
          FONT_CHANNEL_ATTRIBUTE_NAME);
      glBindAttribLocation(program, FONT_PAGE_ATTRIBUTE,
          FONT_PAGE_ATTRIBUTE_NAME);
    }
    if (colorAttr) {
  ...
  protected void loadArmatureUniforms(int program) {
    ...
    if (bonePalete < 0) {
      LOGGER.severe("Bone palete requested but " + BONE_PALETE_UNIFORM
          + " matrixes not active!");
      // throw new AntagonistException();
    }
    if (bonePaleteIT < 0) {
      LOGGER.severe("Bone palete requested but " + BONE_PALETE_IT_UNIFORM
          + " matrixes not active!");
      // throw new AntagonistException();
    }
    assert !Utils.hasGLErrors();
  ...
  protected void loadBasicInstanceUniforms(int program) {
    modelViewProjectionUniform = glGetUniformLocation(program,
        MODEL_VIEW_PROJECTION_UNIFORM);
    ...
    if (modelViewProjectionUniform < 0) {
      LOGGER
          .severe("Basic instance uniforms requested but ModelViewProjection matrix not active!");
      // throw new AntagonistException();
    }
    if (modelViewUniform < 0) {
      LOGGER
          .severe("Basic instance uniforms requested but ModelView matrix not active!");
      // throw new AntagonistException();
    }
    assert !Utils.hasGLErrors();
  }
  ...
  protected void loadSpecialColorsUniforms(int program) {
    ...
    if (specialColor0 < 0) {
      LOGGER
          .severe("Special colors uniforms requested but SpecialColor0 is not active!");
      // throw new AntagonistException();
    }
  ...
  protected void loadBasicLightUniforms(int program) {
    ...
    directionalDirUniform = glGetUniformLocation(program,
        DIRECTIONAL_LIGHT_DIR_UNIFORM);
    directionalColorUniform = glGetUniformLocation(program,
        DIRECTIONAL_LIGHT_COLOR_UNIFORMS);
    if (ambientUniform < 0) {
      LOGGER
          .severe("Basic light uniforms requested but ambient uniform not active!");
      // throw new AntagonistException();
    }
    if (directionalDirUniform < 0) {
      LOGGER
          .severe("Basic light uniforms requested but directional dir uniform not active!");
      // throw new AntagonistException();
    }
    if (directionalColorUniform < 0) {
      LOGGER
          .severe("Basic light uniforms requested but directional color uniform not active!");
      // throw new AntagonistException();
    }
    assert !Utils.hasGLErrors();
  }
  ...
  protected void loadBasicMaterialUniforms(int program) {
    specularFactorUniform = glGetUniformLocation(program,
        SPECULAR_FACTOR_UNIFORM);
    specularGlossinessUniform = glGetUniformLocation(program,
        SPECULAR_GLOSS_UNIFORM);
    heightUniform = glGetUniformLocation(program, HEIGHT_UNIFORM);
    if (specularFactorUniform < 0) {
      LOGGER
          .severe("Basic material uniforms requested but specular factor not active!");
      // throw new AntagonistException();
    }
    if (specularGlossinessUniform < 0) {
      LOGGER
          .severe("Basic material uniforms requested but specular glossiness uniform not active!");
      // throw new AntagonistException();
    }
  }
  ...
  protected void loadFontUniforms(int program) {
    modelViewProjectionUniform = glGetUniformLocation(program,
        FONT_WVP_MATRIX_UNIFORM);
    if (modelViewProjectionUniform < 0) {
      LOGGER
          .severe("Font uniforms requested but worldviewprojection uniform not active!");
      // throw new AntagonistException();
    }
  }
   ...
   * @since 0.1
   */
  private static int textVertexShaderID = -1, textFragmentShaderID = -1;
  ...
  protected int getFontShader(ShaderType shaderType) {
    ...
    case VERTEX:
      if (textVertexShaderID < 0) {
        textVertexShaderID = generateShaderObject(shaderType, Core.getCore()
            .getRenderManager());
    ...
    case FRAGMENT:
      if (textFragmentShaderID < 0) {
        textFragmentShaderID = generateShaderObject(shaderType, Core.getCore()
            .getRenderManager());
   ...
   * @since 0.1
   */
  public static final String textVertexShader = "attribute vec2   a_position;\n"
      + "attribute vec2   a_texCoord;\n"
      + "attribute vec4   a_channel;\n"
      + "attribute float  a_page;\n"
      + "attribute vec3   a_color;\n"
   ...
   * @since 0.1
   */
  public static final String textFragmentShader = "varying vec2  v_texCoord;\n"
      + "varying vec4  v_channel;\n"
      + "varying vec3  v_color;\n"
      + "varying vec4  v_page;\n"
      +
