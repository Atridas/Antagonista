...
public class DebugRenderGL2 extends DebugRender {
  ...
  private void initSphereBuffers() {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, createSphereIndexBuffer(),
        GL_STATIC_DRAW);
  ...
  /**
   * Initializes the buffers to render lines & triangles and calls all other
   * buffer initializers.
   * 
   * @param rm
   ...
   * @since 0.1
   */
  private void renderArrays(int mode, int numElements, Matrix4f model,
      RenderManager rm) {
    debugMaterial.setUpUniforms(rm);

    Technique technique = debugMaterial.getEffect().getTechnique(
        TechniqueType.FORWARD, Quality.MID);
   ...
   * @since 0.1
   */
  private void renderElements(int mode, int numIndices, Matrix4f model,
      RenderManager rm) {
    debugMaterial.setUpUniforms(rm);
  ...
  private void growBuffers() {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, linesBuffer);
    glBufferData(GL_ARRAY_BUFFER, newCapacity * Utils.FLOAT_SIZE,
        GL_DYNAMIC_DRAW);
  }

  /*
   * @Override protected void renderLines(RenderManager rm) { assert !cleaned;
   * 
   * 
   * if(lines.size() == 0) return;
   * 
   * try { float[] vertex = new float[POS_COL_VERTEX_SIZE * 2];
   * 
   * int linesToDraw1 = 0; int linesToDraw2 = 0; buffer1.clear();
   * buffer2.clear();
   * 
   * 
   * for(Line line: lines) {
   * 
   * vertex[0] = line.origin.x; vertex[1] = line.origin.y; vertex[2] =
   * line.origin.z;
   * 
   * vertex[3] = line.color.x; vertex[4] = line.color.y; vertex[5] =
   * line.color.z;
   * 
   * vertex[6] = line.destination.x; vertex[7] = line.destination.y; vertex[8] =
   * line.destination.z;
   * 
   * vertex[9] = line.color.x; vertex[10] = line.color.y; vertex[11] =
   * line.color.z;
   * 
   * if(line.depthEnabled) { buffer1.put(vertex); linesToDraw1++; } else {
   * buffer2.put(vertex); linesToDraw2++; } }
   * 
   * Matrix4f identityMatrix = null; if(linesToDraw1 + linesToDraw2 > 0) {
   * glBindBuffer(GL_ARRAY_BUFFER, linesBuffer); assert !Utils.hasGLErrors();
   * 
   ...
   * 6*Utils.FLOAT_SIZE, 0);
   * glVertexAttribPointer(TechniquePass.COLOR_ATTRIBUTE, 3, GL_FLOAT, false,
   * 6*Utils.FLOAT_SIZE, 3*Utils.FLOAT_SIZE);
   * 
   * assert !Utils.hasGLErrors();
   * 
   * identityMatrix = new Matrix4f(); identityMatrix.setIdentity(); }
   * 
   * if(linesToDraw1>0) { buffer1.flip();
   * 
   ...
   * assert !Utils.hasGLErrors(); }
   * 
   * if(linesToDraw2>0) { buffer2.flip();
   * 
   ...
   * assert !Utils.hasGLErrors(); }
   * 
   * } catch(BufferOverflowException e) { //fem creixer el buffer i rellancem el
   * métode. //brut pq és una classe per fer debug. growBuffers();
   * renderLines(rm); }
   * 
   * }
   */
  ...
  protected void renderLinesAndTriangles(RenderManager rm) {
    ...
    try {
      ...
      if (linesToDraw1 + linesToDraw2 > 0) {
        ...
        glVertexAttribPointer(TechniquePass.POSITION_ATTRIBUTE, 3, GL_FLOAT,
            false, 6 * Utils.FLOAT_SIZE, 0);
        glVertexAttribPointer(TechniquePass.COLOR_ATTRIBUTE, 3, GL_FLOAT,
            false, 6 * Utils.FLOAT_SIZE, 3 * Utils.FLOAT_SIZE);
  ...
  protected void renderCrosses(RenderManager rm) {
    ...
    try {
      ...
      for (Cross cross : crosses) {
        ...
        glVertexAttrib3f(TechniquePass.COLOR_ATTRIBUTE, cross.color.x,
            cross.color.y, cross.color.z);
  ...
  protected void renderSpheres(RenderManager rm) {
    ...
    try {
      ...
      for (Sphere sphere : spheres) {
        ...
        glVertexAttrib3f(TechniquePass.COLOR_ATTRIBUTE, sphere.color.x,
  ...
  protected void renderCircles(RenderManager rm) {
    ...
    try {
      ...
      for (Circle circle : circles) {
        ...
        glVertexAttrib3f(TechniquePass.COLOR_ATTRIBUTE, circle.color.x,
            circle.color.y, circle.color.z);
  ...
  /*
   * @Override protected void renderTriangles(RenderManager rm) { assert
   * !cleaned;
   * 
   * 
   * if(triangles.size() == 0) return;
   * 
   * try { float[] vertex = new float[POS_COL_VERTEX_SIZE * 6];
   * 
   * int trianglesToDraw1 = 0; int trianglesToDraw2 = 0; buffer1.clear();
   * buffer2.clear();
   * 
   * 
   * for(Triangle triangle: triangles) {
   ...
   * vertex[1] = triangle.v0.y; vertex[2] = triangle.v0.z;
   * 
   * vertex[3] = triangle.color.x; vertex[4] = triangle.color.y; vertex[5] =
   * triangle.color.z;
   * 
   * vertex[6] = triangle.v1.x; vertex[7] = triangle.v1.y; vertex[8] =
   * triangle.v1.z;
   * 
   * vertex[9] = triangle.color.x; vertex[10] = triangle.color.y; vertex[11] =
   * triangle.color.z;
   * 
   * 
   * /////////////////////////////////////// vertex[12] = triangle.v0.x;
   * vertex[13] = triangle.v0.y; vertex[14] = triangle.v0.z;
   * 
   * vertex[15] = triangle.color.x; vertex[16] = triangle.color.y; vertex[17] =
   * triangle.color.z;
   * 
   * vertex[18] = triangle.v2.x; vertex[19] = triangle.v2.y; vertex[20] =
   * triangle.v2.z;
   * 
   * vertex[21] = triangle.color.x; vertex[22] = triangle.color.y; vertex[23] =
   * triangle.color.z;
   * 
   * 
   * /////////////////////////////////////// vertex[24] = triangle.v2.x;
   * vertex[25] = triangle.v2.y; vertex[26] = triangle.v2.z;
   * 
   * vertex[27] = triangle.color.x; vertex[28] = triangle.color.y; vertex[29] =
   * triangle.color.z;
   * 
   * vertex[30] = triangle.v1.x; vertex[31] = triangle.v1.y; vertex[32] =
   * triangle.v1.z;
   * 
   * vertex[33] = triangle.color.x; vertex[34] = triangle.color.y; vertex[35] =
   * triangle.color.z;
   * 
   * /////////////////////////////////////// if(triangle.depthEnabled) {
   * buffer1.put(vertex); trianglesToDraw1++; } else { buffer2.put(vertex);
   * trianglesToDraw2++; } }
   * 
   * Matrix4f identityMatrix = null; if(trianglesToDraw1 + trianglesToDraw2 > 0)
   * { glBindBuffer(GL_ARRAY_BUFFER, linesBuffer); assert !Utils.hasGLErrors();
   * 
   * glEnableVertexAttribArray(TechniquePass.POSITION_ATTRIBUTE);
   * glEnableVertexAttribArray(TechniquePass.COLOR_ATTRIBUTE);
   ...
   * identityMatrix = new Matrix4f(); identityMatrix.setIdentity(); }
   * 
   * if(trianglesToDraw1>0) { buffer1.flip();
   * 
   * glEnable(GL_DEPTH_TEST);
   * 
   * glBindBuffer(GL_ARRAY_BUFFER, linesBuffer);
   ...
   * assert !Utils.hasGLErrors(); }
   * 
   * if(trianglesToDraw2>0) { buffer2.flip();
   * 
   * glDisable(GL_DEPTH_TEST);
   * 
   * glBindBuffer(GL_ARRAY_BUFFER, linesBuffer);
  ...
  protected void renderBBs(RenderManager rm) {
    ...
    try {
      ...
      for (AABB aabb : aabbs) {
        ...
        glVertexAttrib3f(TechniquePass.COLOR_ATTRIBUTE, aabb.color.x,
            aabb.color.y, aabb.color.z);
      ...
      for (OBB obb : obbs) {
        ...
        glVertexAttrib3f(TechniquePass.COLOR_ATTRIBUTE, obb.color.x,
            obb.color.y, obb.color.z);
   ...
   * @param rm
   *          RenderManager reference.
   * @since 0.1
   */
  private void setGlobalMatrixes(TechniquePass pass, Matrix4f model,
      RenderManager rm) {
