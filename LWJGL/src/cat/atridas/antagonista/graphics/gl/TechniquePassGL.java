...
public abstract class TechniquePassGL extends TechniquePass {
  private static Logger LOGGER = Logger.getLogger(TechniquePassGL.class
      .getCanonicalName());
  ...
  fontTexture0Uniform, fontTexture1Uniform, fontTexture2Uniform,
      fontTexture3Uniform;
   ...
   * @since 0.1
   */
  private static int defaultVertexShader = -1, defaultFragmentShader = -1,
      defaultGeometryShader = -1, defaultTessControl = -1,
      defaultTessEval = -1;
  ...
  @Override
  protected int completeShaderProgram(int vs, int tc, int te, int gs, int fs,
      RenderManager rm) throws AntagonistException {
    ...
    // uniforms
    if (albedoTexture) {
      albedoTextureUniform = glGetUniformLocation(program,
          ALBEDO_TEXTURE_UNIFORM);
    ...
    if (normalTexture) {
      normalTextureUniform = glGetUniformLocation(program,
          NORMALMAP_TEXTURE_UNIFORM);
    ...
    if (heightTexture) {
      heightTextureUniform = glGetUniformLocation(program,
          HEIGHTMAP_TEXTURE_UNIFORM);
    ...
    if (fontTechnique) {
      fontTexture0Uniform = glGetUniformLocation(program,
          FONT_TEXTURE_0_UNIFORM);
      fontTexture1Uniform = glGetUniformLocation(program,
          FONT_TEXTURE_1_UNIFORM);
      fontTexture2Uniform = glGetUniformLocation(program,
          FONT_TEXTURE_2_UNIFORM);
      fontTexture3Uniform = glGetUniformLocation(program,
          FONT_TEXTURE_3_UNIFORM);
    ...
    if (LOGGER.isLoggable(Level.WARNING)) {
      ...
      int positionLocation = glGetAttribLocation(program,
      ...
      int bitangentLocation = glGetAttribLocation(program,
          BITANGENT_ATTRIBUTE_NAME);
      int uvLocation = glGetAttribLocation(program, UV_ATTRIBUTE_NAME);
      int bonesILocation = glGetAttribLocation(program,
          BLEND_INDEX_ATTRIBUTE_NAME);
      int bonesWLocation = glGetAttribLocation(program,
          BLEND_WEIGHT_ATTRIBUTE_NAME);
      int colorLocation = glGetAttribLocation(program, COLOR_ATTRIBUTE_NAME);

      if (position && positionLocation != POSITION_ATTRIBUTE) {
        LOGGER.warning("Expected position attribute at " + POSITION_ATTRIBUTE
            + " but found in " + positionLocation);
      }
      if (normal && normalLocation != NORMAL_ATTRIBUTE) {
        LOGGER.warning("Expected normal attribute at " + NORMAL_ATTRIBUTE
            + " but found in " + normalLocation);
      }
      if (tangents) {
        if (tangentLocation != TANGENT_ATTRIBUTE)
          LOGGER.warning("Expected tangent attribute at " + TANGENT_ATTRIBUTE
              + " but found in " + tangentLocation);
        if (bitangentLocation != BITANGENT_ATTRIBUTE)
          LOGGER.warning("Expected bitangent attribute at "
              + BITANGENT_ATTRIBUTE + " but found in " + bitangentLocation);
      }
      if (uv && uvLocation != UV_ATTRIBUTE) {
        LOGGER.warning("Expected uv attribute at " + UV_ATTRIBUTE
            + " but found in " + uvLocation);
      }
      if (bones) {
        if (bonesILocation != BLEND_INDEX_ATTRIBUTE)
          LOGGER.warning("Expected bone indexes attribute at "
              + BLEND_INDEX_ATTRIBUTE + " but found in " + bonesILocation);
        if (bonesWLocation != BLEND_WEIGHT_ATTRIBUTE)
          LOGGER.warning("Expected bone weights attribute at "
              + BLEND_WEIGHT_ATTRIBUTE + " but found in " + bonesWLocation);
      }
      // TODO comprovar atributs de les fonts (mandra)

      if (colorAttr) {
        if (colorLocation != COLOR_ATTRIBUTE) {
          LOGGER.warning("Expected color attribute at " + COLOR_ATTRIBUTE
              + " but found in " + colorLocation);
        }
      }
    }
    // int fragDataLoc = glGetFragDataLocation(program,
    // COLOR_FRAGMENT_DATA_NAME);
   ...
   * 
   * @param program
   *          OpenGL program shader identifier.
   * @since 0.1
   */
  protected abstract void bindAttributes(int program);

  /**
   * Loads the basic instance uniforms binding points or sets the correct
   * uniform buffer binding point, depending on the current OpenGL profile.
   * 
   * @param program
   *          OpenGL program shader identifier.
   * @since 0.1
  ...
  /**
   * Loads the armature uniforms binding points or sets the correct uniform
   * buffer binding point, depending on the current OpenGL profile.
   * 
   * @param program
   *          OpenGL program shader identifier.
   * @since 0.3
  ...
  /**
   * Loads the special colors uniforms binding points or sets the correct
   * uniform buffer binding point, depending on the current OpenGL profile.
   * 
   * @param program
   *          OpenGL program shader identifier.
  ...
  /**
   * Loads the basic light uniforms binding points or sets the correct uniform
   * buffer binding point, depending on the current OpenGL profile.
   * 
   * @param program
   *          OpenGL program shader identifier.
  ...
  /**
   * Loads the basic material uniforms binding points or sets the correct
   * uniform buffer binding point, depending on the current OpenGL profile.
   * 
   * @param program
   *          OpenGL program shader identifier.
  ...
  protected int getDefaultShader(ShaderType st) {
    ...
    case VERTEX:
      if (defaultVertexShader < 0) {
        defaultVertexShader = generateShaderObject(st, Core.getCore()
            .getRenderManager());
        compileShader(defaultVertexShader, Core.getCore().getEffectManager()
            .getDefaultShaderSource(st));
      }
      return defaultVertexShader;
    case FRAGMENT:
      if (defaultFragmentShader < 0) {
        defaultFragmentShader = generateShaderObject(st, Core.getCore()
            .getRenderManager());
        compileShader(defaultFragmentShader, Core.getCore().getEffectManager()
            .getDefaultShaderSource(st));
      }
      return defaultFragmentShader;
    case GEOMETRY:
      if (defaultGeometryShader < 0) {
        defaultGeometryShader = generateShaderObject(st, Core.getCore()
            .getRenderManager());
        compileShader(defaultGeometryShader, Core.getCore().getEffectManager()
            .getDefaultShaderSource(st));
      }
      return defaultGeometryShader;
    case TESS_CONTROL:
      if (defaultTessControl < 0) {
        defaultTessControl = generateShaderObject(st, Core.getCore()
            .getRenderManager());
        compileShader(defaultTessControl, Core.getCore().getEffectManager()
            .getDefaultShaderSource(st));
      }
      return defaultTessControl;
    case TESS_EVALUATION:
      if (defaultTessEval < 0) {
        defaultTessEval = generateShaderObject(st, Core.getCore()
            .getRenderManager());
        compileShader(defaultTessEval, Core.getCore().getEffectManager()
            .getDefaultShaderSource(st));
      }
