...
public class DebugRenderGL3 extends DebugRender {
   ...
   * @since 0.1
   */
  private FloatBuffer globalDataBuffer = BufferUtils
      .createFloatBuffer(InstanceBufferUtils.BUFFER_SIZE);
  /**
   * Auxiliar buffers.
   * 
   * @since 0.1
   ...
   * @since 0.1
   */
  private int multipleInstancesGlobalDataBuffer1 = -1,
      instancesColorBuffer1 = -1, multipleInstancesGlobalDataBuffer2 = -1,
      instancesColorBuffer2 = -1;
  ...
  private void initSphereBuffers() {
    ...
    glVertexAttribPointer(TechniquePass.COLOR_ATTRIBUTE, 3, GL_FLOAT, false, 0,
        0);
  ...
  private void initBuffers(RenderManager rm) {
    ...
    if (linesBuffer < 0) {
      ...
      glBufferData(GL_UNIFORM_BUFFER,
          TechniquePass.BASIC_INSTANCE_UNIFORMS_BLOCK_SIZE, GL_DYNAMIC_DRAW);
      ...
      glBindBuffer(GL_ARRAY_BUFFER, instancesColorBuffer1);
      glBufferData(GL_ARRAY_BUFFER, rm.getMaxInstancesBasic() * 3
          * Utils.FLOAT_SIZE, GL_DYNAMIC_DRAW);
      ...
      glBufferData(
          ...
          TechniquePass.BASIC_INSTANCE_UNIFORMS_BLOCK_SIZE
              * rm.getMaxInstancesBasic(), GL_DYNAMIC_DRAW);
   ...
   *          number of vertices to render.
   * @param numInstances
   *          number of different instances to render.
   * @param rm
   *          Render Manager reference.
   * @since 0.1
   */
  private void renderArraysInstanced(int mode, int numElements,
      int numInstances, RenderManager rm) {
    debugMaterial.setUpUniforms(rm);
   ...
   * @since 0.1
   */
  private void renderElementsInstanced(int mode, int numIndices,
      int numInstances, RenderManager rm) {
    debugMaterial.setUpUniforms(rm);
    ...
    for (TechniquePass pass : technique.getPasses()) {
      ...
      glDrawElementsInstanced(mode, numIndices, GL_UNSIGNED_SHORT, 0,
          numInstances);
    }
    assert !Utils.hasGLErrors();
  }
   ...
   * 
   * if(linesToDraw1 + linesToDraw2 > 0) { setGlobalMatrixes(rm);
   * glBindVertexArray(linesVAO); }
   * 
   ...
   * 
   * glBindBuffer(GL_ARRAY_BUFFER, linesBuffer); glBufferData(GL_ARRAY_BUFFER,
   * buffer1, GL_DYNAMIC_DRAW);
   * 
  ...
  protected void renderCrosses(RenderManager rm) {
    ...
    try {
      ...
      if (crossesToDraw1 > 0) {
        ...
        drawInstanced(buffer1, buffer2, crossesToDraw1, false, true, GL_LINES,
            crossesNumVertexs, rm);
      }
      ...
      if (crossesToDraw2 > 0) {
        ...
        drawInstanced(buffer3, buffer4, crossesToDraw2, false, false, GL_LINES,
            crossesNumVertexs, rm);
      }
  ...
  protected void renderSpheres(RenderManager rm) {
    ...
    try {
      ...
      if (spheresToDraw1 > 0) {
        ...
        drawInstanced(buffer1, buffer2, spheresToDraw1, true, true,
      ...
      if (spheresToDraw2 > 0) {
        ...
        drawInstanced(buffer3, buffer4, spheresToDraw2, true, false,
            GL_LINE_STRIP, sphereNumIndices, rm);
  ...
  protected void renderCircles(RenderManager rm) {
    ...
    try {
      ...
      if (circlesToDraw1 > 0) {
        ...
        drawInstanced(buffer1, buffer2, circlesToDraw1, false, true,
      ...
      if (circlesToDraw2 > 0) {
        ...
        drawInstanced(buffer3, buffer4, circlesToDraw2, false, false,
            GL_LINE_LOOP, circlesNumVertexs, rm);
      }
  ...
  protected void renderAxes(RenderManager rm) {
    ...
    try {
      ...
      if (axesToDraw1 > 0) {
        ...
        drawInstanced(null, buffer1, axesToDraw1, false, true, GL_LINES,
            axesNumVertexs, rm);

        // glEnable(GL_DEPTH_TEST);

        // glBindBuffer(GL_UNIFORM_BUFFER,
        // multipleInstancesGlobalDataBuffer1);//TODO
      ...
      if (axesToDraw2 > 0) {
        ...
        drawInstanced(null, buffer2, axesToDraw2, false, false, GL_LINES,
            axesNumVertexs, rm);
  ...
  protected void renderBBs(RenderManager rm) {
    ...
    try {
      ...
      if (bbToDraw1 > 0) {
        ...
        drawInstanced(buffer1, buffer2, bbToDraw1, true, true, GL_LINES,
            bbNumIndices, rm);
      ...
      if (bbToDraw2 > 0) {
        ...
        drawInstanced(buffer3, buffer4, bbToDraw2, true, false, GL_LINES,
            bbNumIndices, rm);
      }
  ...
  private void drawInstanced(FloatBuffer colorBuffer, FloatBuffer matrixBuffer,
      int instances, boolean elements, boolean depthTest, int primitiveType,
      int numIndices, RenderManager rm) {
    assert colorBuffer == null
        || colorBuffer.limit() == instances * colorBufferInstanceSize;
