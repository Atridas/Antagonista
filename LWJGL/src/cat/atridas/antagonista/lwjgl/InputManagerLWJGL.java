...
/**
 * Input manager class. This class is responsible of registering all user input
 * actions and map them to a configuration specified events.
 * 
...
public final class InputManagerLWJGL extends InputManager {
  private static final Logger LOGGER = Logger.getLogger(InputManagerLWJGL.class
      .getCanonicalName());
   ...
   *           on severe errors such as file not found, or ill-formated xml.
   * @since 0.1
   */
      ...
      for (int i = 0; i < modesXML.getLength(); ++i) {
        ...
        for (int j = 0; j < eventsXML.getLength(); ++j) {
          ...
          try {
            ...
            registerAction(new HashedString(name), hsMode, new Event(et, ea,
                keyID));
  ...
  /**
   * Gets the mouse pixel position in the X axis. The 0 coordinate is in the
   * left of the screen.
   * 
  ...
  /**
   * Gets the mouse pixel position in the Y axis. The 0 coordinate is in the
  ...
  /**
   * Gets the mouse pixel displacement in the X axis during the last frame. A
   * positive displacement is a movement to the right.
   * 
  ...
  /**
   * Gets the mouse pixel displacement in the Y axis during the last frame. A
   * positive displacement is a movement to the top of the screen.
   * 
  ...
  /**
   * Gets the mouse wheel displacement during the last frame. A positive
   * displacement means that the wheel has been rotated in the direction to the
   * user.
   * 
  ...
  /**
   * Checks if an action is active. If an action is active, but it's mode is
   * not, said action is treated as non-active.
   * 
   * @param action
   *          identifier.
  ...
  public boolean isActionActive(final HashedString action) {
    return activeActions.containsKey(action)
        && activeModes.contains(actionToMode.get(action));
  }

  /**
   * Gets the value of an active action. It is an error to call this method
   * using and inactive action identifier and the method may throw and exception
   * in that case. The action value is 1 in single frame actions, the last frame
   * pixel mouse displacement in mouse movement events and the time since last
   * state change in key/button presses/unpresses.
   * 
   * @param action
   *          identifier.
  ...
  public void update(DeltaTime dt) {
    ...
    deltaMouseX = deltaMouseY = deltaMouseZ = 0;
    ...
    while (Mouse.next()) {
      if (Mouse.getEventButton() == -1) {
        ...
        if (deltaMouseX != 0) {
          ...
          action = actions.get(new Event(EventType.MOUSE, EventAction.MOVE,
        ...
        if (Mouse.getEventButtonState()) {
          action = actions.get(new Event(EventType.MOUSE, EventAction.UP_DOWN,
              button));
          actionPermanent = actions.get(new Event(EventType.MOUSE,
              EventAction.DOWN, button));
          actionPermanentToRemove = actions.get(new Event(EventType.MOUSE,
              EventAction.UP, button));
        } else {
          action = actions.get(new Event(EventType.MOUSE, EventAction.DOWN_UP,
              button));
          actionPermanent = actions.get(new Event(EventType.MOUSE,
              EventAction.UP, button));
          actionPermanentToRemove = actions.get(new Event(EventType.MOUSE,
    ...
    while (Keyboard.next()) {
      ...
      if (Keyboard.getEventKeyState()) {
        action = actions.get(new Event(EventType.KEYBOARD, EventAction.UP_DOWN,
            key));
        actionPermanent = actions.get(new Event(EventType.KEYBOARD,
            EventAction.DOWN, key));
        actionPermanentToRemove = actions.get(new Event(EventType.KEYBOARD,
            EventAction.UP, key));
      } else {
        action = actions.get(new Event(EventType.KEYBOARD, EventAction.DOWN_UP,
            key));
        actionPermanent = actions.get(new Event(EventType.KEYBOARD,
            EventAction.UP, key));
        actionPermanentToRemove = actions.get(new Event(EventType.KEYBOARD,
  ...
  public static enum EventAction {
     ...
     * @throws IllegalArgumentException
     *           if the string does not represent any value.
     * @since 0.1
     */
