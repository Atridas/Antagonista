 ...
 * 
 */
public final class PhysicsWorldBullet implements PhysicsWorld {
  ...
  private Dispatcher collisionDispatcher = new CollisionDispatcher(
  ...
  public void debugDraw() {
    ...
    // wireframe drawing seems to not be implemented correctly in jBullet, so I
    // draw it here.
    if ((dynamicsWorld.getDebugDrawer().getDebugMode() & DebugDrawModes.DRAW_WIREFRAME) != 0) {
      ...
      Vector3f[] triangle = new Vector3f[] { new Vector3f(), new Vector3f(),
          new Vector3f() };
      ...
      for (CollisionObject co : dynamicsWorld.getCollisionObjectArray()) {
        ...
        if (cs instanceof SphereShape) {
          ...
          StridingMeshInterface meshInterface = triangleMeshShape
              .getMeshInterface();
          ...
          for (int i = 0; i < meshInterface.getNumSubParts(); ++i) {
            ...
            for (int j = 0; j < vd.getIndexCount() / 3; j++) {
              ...
              dr.addTriangle(point1, point2, point3, userInfo.color,
                  userInfo.zTest);
            }
          ...
          /*
           * transform1.origin.set(0,0,0); transform1.basis.setIdentity();
           * 
           * cShape.getAabb(transform, vector1, vector2);
           * 
           * //transform.transform(vector1); //transform.transform(vector2);
           * 
           * point1.set(vector1); point2.set(vector2);
           * 
           * dr.addAABB(point1, point2, userInfo.color, userInfo.zTest);
           * 
           * point1.set(.5f,.5f,2); transform.getMatrix(matrix1);
           * dr.addOBB(matrix1, point1, new Color3f(0,1,1), userInfo.zTest);
           */

          /*
           * cShape.getBoundingSphere(vector1, faux1);
           * transform.transform(vector1); point1.set(vector1);
          ...
          throw new RuntimeException("Debug draw of shape "
              + cs.getClass().getCanonicalName() + " is not yet implemented!");
        }
      }
    }
  }
  ...
  public StaticRigidBody createStaticRigidBody(PhysicShape _physicShape,
      Vector3f fromGameToBulletOffset, PhysicsUserInfo _userInfo,
      Transformation _bodyTransform) {
    return createStaticRigidBody((PhysicShapeBullet) _physicShape,
        fromGameToBulletOffset, _userInfo, _bodyTransform);
  }

  public StaticRigidBody createStaticRigidBody(PhysicShapeBullet _physicShape,
      Vector3f fromGameToBulletOffset, PhysicsUserInfo _userInfo,
      Transformation _bodyTransform) {
    ...
    RigidBodyConstructionInfo rbci = new RigidBodyConstructionInfo(0, dms,
        _physicShape.getBulletShape());
    // RigidBodyConstructionInfo rbci = new RigidBodyConstructionInfo(0, dms,
    // new SphereShape(5));
  ...
  public KinematicCharacter createKinematicCharacter(float characterWidth,
      float characterHeight, Transformation _bodyTransform, float stepHeight,
    ...
    // TODO sweepBP.getOverlappingPairCache().setInternalGhostPairCallback(new
    // GhostPairCallback());
    // float characterHeight = 1.75f * characterScale;
    // float characterWidth = 1.75f * characterScale;

    ConvexShape capsule = new CapsuleShapeZ(characterWidth * .5f,
        characterHeight * .5f + characterWidth);
    // ConvexShape capsule = new CylinderShapeZ(new Vector3f(characterWidth/2,
    // 0, characterHeight));
    ...
    KinematicCharacterController character = new KinematicCharacterController(
        ghostObject, capsule, stepHeight);

    character.setUpAxis(2);
    // TODO new
    // BspToBulletConverter().convertBsp(getClass().getResourceAsStream("/com/bulletphysics/demos/bsp/exported.bsp.txt"));
    ...
    dynamicsWorld
        .addCollisionObject(
            ...
            (short) (CollisionFilterGroups.STATIC_FILTER
                | CollisionFilterGroups.DEFAULT_FILTER | CollisionFilterGroups.CHARACTER_FILTER));
  ...
  // ClosestRayResultCallback raycastCRRC = new
  // ClosestRayResultCallback(raycastOrigin, raycastDestiny);
  public PhysicsUserInfo raycast(Point3f origin, Point3f destiny,
      Point3f point_, Vector3f normal_) {

    raycastOrigin.set(origin);
    raycastDestiny.set(destiny);

    ClosestRayResultCallback raycastCRRC = new ClosestRayResultCallback(
        raycastOrigin, raycastDestiny);
