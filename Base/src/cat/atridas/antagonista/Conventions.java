...
/**
 * <p>
 * This class encapsulates information about the front, up, left and right
 * vectors in the engine. Use the vectors here created and never change them.
 * Also, whenever you need to compute the Euler angles, use the functions in
 * this class so everything works consistently.
 * </p>
 * <p>
 * We will do a Yaw -> Pitch -> Roll rotation (in that order) and Yaw will be
 * arround the Z vector (a positive Yaw means turning to the left), Pitch
 * arround the -X vector (a positive angle means to turn your head up) and Roll
 * arround the -Y angle (a positive Roll means to do a barrell roll to your
 * right).
 * </p>
 * <p>
 * In the Blender exporter we should take the ZXY rotation and negate both Pitch
 * and Roll.
 * </p>
 * 
...
public class Conventions {
   ...
   * @since 0.1
   * @param _quaternion
   *          Rotation entered as a unit quaternion.
   * @param euler_
   *          returns the angles in Yaw (x) Pitch (y) Roll (z) convention.
   */
  public static void quaternionToEulerAngles(Quat4f _quaternion, Tuple3f euler_) {
    ...
    double abs = Math.abs(1 - q00 - q11 - q22 - q33); // que estigui normalitzat
                                                      // per favor
    assert abs < 0.0001;
   ...
   * @since 0.1
   * @param _euler
   *          Euler rotation to translate, in Yaw (x) Pitch (y) Roll (z)
   *          convention.
   * @param quaternion_
   *          output.
   */
