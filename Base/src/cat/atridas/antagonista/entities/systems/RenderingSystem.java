...
public class RenderingSystem implements cat.atridas.antagonista.entities.System {
  ...
  private RenderableObjectManager rom = Core.getCore()
      .getRenderableObjectManager();

  private static ThreadLocal<Transformation> g_transAux = new ThreadLocal<Transformation>() {
    @Override
    protected Transformation initialValue() {
  ...
  public void addEntity(Entity entity, Component<?>[] components,
    ...
    RenderableObject ro = rom.addRenderableObject(entity.getId(),
        mesh.getMesh());
