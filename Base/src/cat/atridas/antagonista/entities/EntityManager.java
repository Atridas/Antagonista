...
public final class EntityManager {
  ...
  /**
   * Counter indicating the number of entities created with automatic name
   * generation.
   * 
   * @since 0.2
   */
  ...
  /**
   * Map from entityIDs -> ComponentIDs indicating what components a given
   * entity has.
   * 
   * @since 0.2
   */
  ...
  /**
   * Contains all entities that had been updated (created, deleted or with
   * new/erased components)
   * 
   * @since 0.2
   */
   ...
   * 
   * @param entity
   *          id of the entity that will have the component.
   * @param component
   *          id of the component to create.
   * @return a new component.
   * @since 0.2
   */
  public synchronized <T extends GlobalComponent<?>> T createComponent(
      Entity entity, HashedString component) {
    // TODO fer un read/write lock
    assert entities.containsKey(entity.getId());
    assert components.containsKey(component);

    HashMap<HashedString, GlobalComponent<?>> componentMap = components
        .get(component);
   ...
   * @return a component or <code>null</code> if this entity does not have this
   *         component.
   * @since 0.2
   */
  @SuppressWarnings("unchecked")
  public synchronized <T extends GlobalComponent<?>> T getComponent(
      Entity entity, HashedString component) {
    // TODO fer un read/write lock
    ...
    assert (componentToReturn != null && entityComponentCache.get(
        entity.getId()).contains(component))
        || (componentToReturn == null && !entityComponentCache.get(
            entity.getId()).contains(component));

    assert componentToReturn == null
        || componentToReturn.getComponentType().equals(component);
    assert componentToReturn == null
        || componentToReturn.getEntityId().equals(entity.getId());
   ...
   * @since 0.2
   */
  public <T extends GlobalComponent<?>> void registerComponentType(
      Class<T> component) {
    try {
      ...
      if (componentTypes.containsKey(identifier)) {
        LOGGER.severe("Registering a component with a duplicated identifier: "
            + identifier);
        throw new RuntimeException();
      }

      componentTypes.put(identifier, component);
      components.put(identifier,
          new HashMap<HashedString, GlobalComponent<?>>());
    } catch (Exception e) {
      LOGGER.severe(Utils.logExceptionStringAndStack(e));
      throw new RuntimeException(e);
    }
  ...
  /**
   * Checks what entites had been updated (created, deleted or with new/erased
   * components) since the last call on this method and returns a set of them.
   * 
   * @param updatedEntities_
   *          output parameter with all entities that had been updated.
   * @since 0.2
   */
