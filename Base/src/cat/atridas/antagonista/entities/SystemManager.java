...
public class SystemManager {
  public static final HashedString renderInteface = new HashedString(
      "Render Interface");
  public static final HashedString inputInteface = new HashedString(
      "Input Interface");
  public static final HashedString physicsInteface = new HashedString(
      "Physics Interface");
   ...
   * 
   * This has for each system a set of all entities, and for each entity an
   * array of each local component.
   * 
   * @since 0.2
   */
  ...
  public void registerSystem(System system) {
    ...
    entitiesCache.put(system.getSystemId(),
        new HashMap<HashedString, CachedEntity>());
    acumulatedEntityChanges.put(system.getSystemId(),
  ...
  private void updateSystem(System system, EntityManager em, DeltaTime dt) {
    HashSet<HashedString> systemAcumulatedEntityChanges = acumulatedEntityChanges
        .get(system.getSystemId());

    HashMap<HashedString, CachedEntity> systemEntities = entitiesCache
        .get(system.getSystemId());
    ...
    if (systemAcumulatedEntityChanges.size() > 0) {
      ...
      for (HashedString entityId : systemAcumulatedEntityChanges) {
        ...
        boolean thisEntityHasAllNeededComponents = entityComponents
            .containsAll(systemComponents);

        if (!thisSystemHasThisEntity && thisEntityHasAllNeededComponents) {
          // afegir entitat.

          CachedEntity cachedEntity = new CachedEntity(entity,
              componentsUsed.size() + optionalComponents.size());

          for (int i = 0; i < componentsUsed.size(); ++i) {
            cachedEntity.localComponents[i] = (LocalComponent<?>) em
                .getComponent(entity, componentsUsed.get(i)).createLocalCopy();
            assert cachedEntity.localComponents[i].isInitialized();
          }
          int dif = componentsUsed.size();

          for (int i = 0; i < optionalComponents.size(); ++i) {
            GlobalComponent<?> optional = em.getComponent(entity,
                optionalComponents.get(i));
            if (optional != null) {
              cachedEntity.localComponents[i + dif] = optional
                  .createLocalCopy();
              assert cachedEntity.localComponents[i].isInitialized();
            }
          }
    ...
    for (Entry<HashedString, CachedEntity> systemEntity : systemEntities
        .entrySet()) {
      ...
      for (int i = 0; i < optionalComponents.size(); ++i) {
        if (cachedEntity.localComponents[i + dif] == null) {
          GlobalComponent<?> optional = em.getComponent(cachedEntity.entity,
              optionalComponents.get(i));
          if (optional != null) {
      ...
      system
          .updateEntity(cachedEntity.entity, cachedEntity.localComponents, dt);
    }
  }
  ...
  private synchronized void pushSystemChanges(System system) {
    ...
    for (CachedEntity cachedEntity : systemEntities.values()) {
      for (LocalComponent<?> component : cachedEntity.localComponents) {
        if (component != null
            && writeToComponents.contains(component.getComponentType()))
          component.pushChanges();
      }
    }
  }

  public static boolean assertSystemInputParameters(Entity entity,
      Component<?>[] components, System system) {
