...
public class ArmatureCore extends Resource {
  private static Logger LOGGER = Logger.getLogger(ArmatureCore.class
      .getCanonicalName());
  ...
  private BoneCore loadBone(String[] lines, BoneCore parent) {
    ...
    BoneCore bone = new BoneCore(id, transformMatrix, numChildren, parent,
        bones.size());
  ...
  public void debugRender(DebugRender dr, Matrix4f worldMatrix) {
    ...
    Font font = Core.getCore().getFontManager()
        .getResource(FontManager.FONT_14);
    ...
    for (BoneCore bone : bones) {
      ...
      aux.get(translation);
      dr.addString(translation, font, bone.getName().toString(), .1f,
          Utils.BLACK, false);
    }
  ...
  public static class BoneCore {
    ...
    private BoneCore(HashedString _name, Matrix4f _transformMatrix,
        int numChildren, BoneCore _parent, int _index) {
      name = _name;
