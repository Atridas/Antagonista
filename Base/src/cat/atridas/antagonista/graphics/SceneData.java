...
public abstract class SceneData {
   ...
   * 
   * @param fovy
   *          angle (in degrees) of the camera in the y plane.
   * @param zNear
   *          depth plane
   * @param zFar
   *          depth plane
  ...
  public final void setPerspective(float fovy, float zNear, float zFar) {
    setPerspective(fovy, (float) rm.getWidth() / (float) rm.getHeight(), zNear,
        zFar);
  }
   ...
   * @see #setPerspective(float, float, float, float)
   */
  public final void setPerspective(float fovy, float aspect, float zNear,
      float zFar) {
    float f = (float) (1.0 / Math.tan((fovy / 2.) * (Math.PI / 180.f)));
    projectionMatrix.setColumn(0, f / aspect, 0, 0, 0);
    projectionMatrix.setColumn(1, 0, f, 0, 0);
    projectionMatrix.setColumn(2, 0, 0, (zFar + zNear) / (zNear - zFar), -1);
    projectionMatrix.setColumn(3, 0, 0, 2 * zFar * zNear / (zNear - zFar), 0);
   ...
   * @since 0.1
   */
  public final void setFrustum(float _left, float _right, float _bottom,
      float _top, float _near, float _far) {
    projectionMatrix.setColumn(0, 2 * _near / (_right - _left), 0.0f, 0.0f,
        0.0f);
    projectionMatrix.setColumn(1, 0.0f, 2 * _near / (_top - _bottom), 0.0f,
        0.0f);
    projectionMatrix.setColumn(2, (_right + _left) / (_right - _left),
        (_top + _bottom) / (_top - _bottom), -(_far + _near) / (_far - _near),
        -1.0f);
    projectionMatrix.setColumn(3, 0.0f, 0.0f, -2 * _near * _far
        / (_far - _near), 0.0f);
   ...
   * @since 0.1
   */
  public final void setOrtho(float _left, float _right, float _top,
  ...
  /**
   * Set an orthogonal projection going from 0 to the current screen width an
   * heights.
   * 
   * @param _near
   *          plane
  ...
  /**
   * Saves an orthogonal projection similar to {@link #setOrtho(float, float)}
   * into a matrix.
   * 
   * @param near
   *          plane
   ...
   * @see #setOrtho(float, float, float, float, float, float)
   */
  public static final void getOrtho(float left, float right, float top,
      float bottom, float near, float far, Matrix4f matrix) {
    matrix.setColumn(0, 2 / (right - left), 0, 0, 0);
    matrix.setColumn(1, 0, 2 / (top - bottom), 0, 0);
    matrix.setColumn(2, 0, 0, -2 / (far - near), 0);
    matrix.setColumn(3, -(right + left) / (right - left), -(top + bottom)
  ...
  public final void setCamera(Point3f eye, Point3f lookAt, Vector3f up) {
    ...
    viewMatrix.setColumn(3, cameraPosition.x, cameraPosition.y,
  ...
  /**
   * Gets the transformation matrix that transforms from world space to
   * homogeneous space.
   * 
  ...
  /**
   * Gets the transformation matrix that transforms from world space to view
   * space.
   * 
   * @param view_
   *          output parameter.
  ...
  /**
   * Gets the transformation matrix that transforms from view space to
   * homogeneous space.
   * 
   * @param projection_
   *          output parameter.
  ...
  /**
   * Gets the inverse matrix that transforms from homogeneus space to world
   * space.
   * 
   * @param inverseVPMatrix_
   *          output parameter.
   * @since 0.2
   */
  ...
  /**
   * Gets the point where the line from the camera defined with the window
   * coordinates intersects the far plane.
   * 
   * @param _windowPoint
   *          window coordinates. They range from (0,0) at the bottom left to
   *          (width, height) at the top right.
   * @param worldPoint_
   *          return parameter. Point in world coordinates.
   * @since 0.2
   */
