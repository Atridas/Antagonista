...
public abstract class TechniquePass {
  private static Logger LOGGER = Logger.getLogger(TechniquePass.class
      .getCanonicalName());
   ...
   * @since 0.1
   */
  public static final int POSITION_ATTRIBUTE = 0, NORMAL_ATTRIBUTE = 1,
      TANGENT_ATTRIBUTE = 2, BITANGENT_ATTRIBUTE = 3, UV_ATTRIBUTE = 4,
      BLEND_INDEX_ATTRIBUTE = 5, BLEND_WEIGHT_ATTRIBUTE = 6,
      COLOR_ATTRIBUTE = 7;
  ...
  public static final String POSITION_ATTRIBUTE_NAME = "a_v3Position",
      ...
      BITANGENT_ATTRIBUTE_NAME = "a_v3Bitangent", UV_ATTRIBUTE_NAME = "a_v2UV",
   ...
   * @since 0.1
   */
  public static final int ALBEDO_TEXTURE_UNIT = 0, NORMALMAP_TEXTURE_UNIT = 1,
   ...
   * @since 0.1
   */
  public static final int BASIC_INSTANCE_UNIFORMS_BLOCK_SIZE = Utils.FLOAT_SIZE
      * (4 * 4) * 3;
  // public static final String BASIC_INSTANCE_UNIFORMS_STRUCT =
  // "u_InstanceInfo";
  /**
   ...
   * @since 0.3
   */
  public static final int ARMATURE_UNIFORMS_BLOCK_SIZE = Utils.FLOAT_SIZE
      * (3 * 4) * MAX_BONES * 2;
  // public static final String BASIC_INSTANCE_UNIFORMS_STRUCT =
  // "u_InstanceInfo";
  /**
  ...
  public static final String SPECULAR_FACTOR_UNIFORM = "u_fSpecularFactor",
      SPECULAR_GLOSS_UNIFORM = "u_fGlossiness", HEIGHT_UNIFORM = "u_fHeight";
   ...
   * @since 0.1
   */
  public static final int FONT_POSITION_ATTRIBUTE = 0, FONT_TEX_ATTRIBUTE = 1,
      FONT_CHANNEL_ATTRIBUTE = 2, FONT_PAGE_ATTRIBUTE = 3,
   ...
   * @since 0.1
   */
  public static final int FONT_TEXTURE_0_UNIT = 0, FONT_TEXTURE_1_UNIT = 1,
      FONT_TEXTURE_2_UNIT = 2, FONT_TEXTURE_3_UNIT = 3;
  ...
  public static final String FONT_TEXTURE_0_UNIFORM = "u_page0",
      FONT_TEXTURE_1_UNIFORM = "u_page1", FONT_TEXTURE_2_UNIFORM = "u_page2",
  ...
  /**
   * Maximum number of instances that the current memory limitations allow to
  ...
  protected boolean albedoTexture, normalTexture, heightTexture,
      basicInstanceUniforms, armatureUniforms, specialColorsUniforms,
      basicLight, basicMaterial;
  ...
  /**
   * <code>true</code> if this program should change the depth test status when
   * activated.
   * 
   * @since 0.1
   * @see #depthTestStatus
  ...
  /**
   * <code>true</code> if this program should change the depth writing status
   * when activated.
   * 
   * @since 0.1
   * @see #zWrite
  ...
  /**
   * New depth function to set when this program is activated. Is this field is
   * null, this program should not change the depth function.
   * 
   * @since 0.1
  ...
  /**
   * <code>true</code> if this program should change alpha blending status when
   * activated.
   * 
   * @since 0.1
  ...
  /**
   * New alpha blending operation to set when this program is activated. If this
   * field is null, this program should not change the alpha blending operation.
   * 
   * @since 0.1
  ...
  /**
   * New alpha blending status when this program is activated. This variable
   * maps each render target with each new blending status.
   * 
   * @since 0.1
   ...
   * New alpha blending operation to set when this program is activated. If this
   * field is null, this program should not change the alpha blending operation.
   * This variable maps each render target with each new blending operation.
   * 
   * @since 0.1
  ...
  protected TechniquePass() {
    ...
    changeDepthTest = depthTestStatus = changeAlphaBlending = position = colorResult = true;
  ...
  protected TechniquePass(boolean fontShader) {
    ...
    fontTechnique = changeZWrite = changeAlphaBlending = alphaBlendingActive = true;

    changeDepthTest = zWrite = false;
  ...
  /**
   * Compiles a shader source. If there are any errors, this method logs them
   * and returns a default shader.
   * 
   * @param shaderXML
   *          element containing the resource name of the desired shader.
   * @param st
   *          shader stage.
   * @param em
  ...
  private int loadShader(Element shaderXML, ShaderType st, EffectManager em,
    ...
    String resourceName = Utils.getStringContentFromXMLSubElement(shaderXML,
        "resource");

    String shaderSource = em
        .getShaderSource(new HashedString(resourceName), st);
  ...
  private void loadRenderStates(Element renderStatesXML) {
    ...
    for (int i = 0; i < nl.getLength(); ++i) {
      ...
      FIRST_SWITCH: switch (element.getTagName()) {
      case "depth_test":
        if (LOGGER.isLoggable(Level.CONFIG))
      ...
      case "alpha_blending":
        ...
        if (element.hasAttribute("render_target")) {
          int renderTarget = Integer.parseInt(element
              .getAttribute("render_target"));
      ...
      case "blend_func":
        ...
        if (element.getElementsByTagName("source").getLength() > 1) {
          ...
          String src = Utils.getStringContentFromXMLSubElement(element,
              "source");
          String dst = Utils.getStringContentFromXMLSubElement(element,
              "destination");
          ...
          String srcColor = Utils.getStringContentFromXMLSubElement(element,
              "source_color");
          String dstColor = Utils.getStringContentFromXMLSubElement(element,
              "destination_color");
          String srcAlpha = Utils.getStringContentFromXMLSubElement(element,
              "source_alpha");
          String dstAlpha = Utils.getStringContentFromXMLSubElement(element,
              "destination_alpha");
        ...
        if (element.hasAttribute("render_target")) {
          ...
          if (op != null) {
            ...
            assert !alphaBlendingOperationSeparatePerRenderTarget
                .containsKey(renderTarget);
            assert !alphaBlendingOperationPerRenderTarget
                .containsKey(renderTarget);
            ...
            alphaBlendingOperationSeparatePerRenderTarget
                .put(renderTarget, ops);
          }
   ...
   * 
   * @param shaderID
   *          OpenGL shader stage identifier.
   * @since 0.1
   */
   ...
   * 
   * @param vs
   *          vertex shader OpenGL identifier.
   * @param tc
   *          tesselation control shader OpenGL identifier.
   * @param te
   *          tesselation evaulation shader OpenGL identifier.
   * @param gs
   *          geometry shader OpenGL identifier.
   * @param fs
   *          fragment shader OpenGL identifier.
   * @param rm
   *          Render Manager reference.
   * @return the successfully linked shader program OpenGL identifier.
   * @throws AntagonistException
   *           if the linking operation fails.
   * @since 0.1
   */
  protected abstract int completeShaderProgram(int vs, int tc, int te, int gs,
      int fs, RenderManager rm) throws AntagonistException;

  /**
   * Deletes a shader program.
   * 
   * @param shaderProgramID
   *          shader program OpenGL identifier.
   * @since 0.1
   */
  ...
  /**
   * Activates this program pass. This method sets the shader program active and
   * changes some render states according to the configuration.
   * 
   * @param rm
   *          Render Manager reference.
   * @since 0.1
  ...
  public void activate(RenderManager rm) {
    ...
    if (changeAlphaBlending)
      ...
      for (Entry<Integer, Boolean> alphaActive : alphaBlendingPerRenderTarget
          .entrySet()) {
    ...
    if (alphaOperation != null)
      ...
      for (Entry<Integer, BlendOperation> blendOperation : alphaBlendingOperationPerRenderTarget
          .entrySet()) {
        rm.setAlphaBlend(blendOperation.getValue(), blendOperation.getKey());
      }
      for (Entry<Integer, BlendOperationSeparate> blendOperation : alphaBlendingOperationSeparatePerRenderTarget
          .entrySet()) {
        rm.setAlphaBlend(blendOperation.getValue(), blendOperation.getKey());
      }
    }
