...
public abstract class Mesh extends Resource {
  private static Logger LOGGER = Logger
      .getLogger(Mesh.class.getCanonicalName());
  ...
  /**
   * Number of elements in a vertex animated. Short indices count as half
   * element.
   * 
   * @since 0.1
   */
  public static final int NUM_ELEMENTS_PER_VERTEX_ANIMATED_MESH = 14 + (2 + 4); // 4
                                                                                // indexos
                                                                                // (shorts)
                                                                                // +
                                                                                // 4
   ...
   * @since 0.1
   */
  protected static final int STATIC_MESH_STRIDE = NUM_ELEMENTS_PER_VERTEX_STATIC_MESH
      * Utils.FLOAT_SIZE,
      ANIMATED_MESH_STRIDE = NUM_ELEMENTS_PER_VERTEX_ANIMATED_MESH
          * Utils.FLOAT_SIZE;
  ...
  /**
   * Number of submeshes this mesh has. Each submesh has an index buffer and a
   * different material.
   * 
   * @since 0.1
   */
  ...
  private boolean loadText(InputStream is) {
    ...
    try {
      ...
      for (int i = 0; i < numVerts; ++i) {
        ...
        if (animated) {
          float w1 = Float
              .parseFloat(elements[NUM_ELEMENTS_PER_VERTEX_STATIC_MESH + 0]);
          float w2 = Float
              .parseFloat(elements[NUM_ELEMENTS_PER_VERTEX_STATIC_MESH + 1]);
          float w3 = Float
              .parseFloat(elements[NUM_ELEMENTS_PER_VERTEX_STATIC_MESH + 2]);
          float w4 = Float
              .parseFloat(elements[NUM_ELEMENTS_PER_VERTEX_STATIC_MESH + 3]);

          short i1 = Short
              .parseShort(elements[NUM_ELEMENTS_PER_VERTEX_STATIC_MESH + 4]);
          short i2 = Short
              .parseShort(elements[NUM_ELEMENTS_PER_VERTEX_STATIC_MESH + 5]);
          short i3 = Short
              .parseShort(elements[NUM_ELEMENTS_PER_VERTEX_STATIC_MESH + 6]);
          short i4 = Short
              .parseShort(elements[NUM_ELEMENTS_PER_VERTEX_STATIC_MESH + 7]);

          /*
           * if(w1 > 0) { i1--; } if(w2 > 0) { i2--; } if(w3 > 0) { i3--; }
           * if(w4 > 0) { i4--; }
           */
          ...
          assert Math.abs(w1 + w2 + w3 + w4 - 1.) < Utils.EPSILON
              || w1 + w2 + w3 + w4 == 0f;
        }
      ...
      if (animated) {
        vertexBuffer.limit(numVerts * NUM_ELEMENTS_PER_VERTEX_ANIMATED_MESH
            * Utils.FLOAT_SIZE);
      } else {
        vertexBuffer.limit(numVerts * NUM_ELEMENTS_PER_VERTEX_STATIC_MESH
            * Utils.FLOAT_SIZE);
      }
      faces.position(0);
  ...
  /**
   * Creates the OpenGL buffers. This method must create the ARRAY_BUFFER, the
   * ELEMENT_ARRAY_BUFFER and the vertex array object if possible (OpenGL 3.0
   * and greater).
   * 
   * @param vertexBuffer
   *          vertex buffer.
   ...
   *          index buffer.
   * @param animated
   *          indicates if the mesh is animated or not.
   ...
   *           if the <code>_submesh</code> parameter is not a valid index.
   * @since 0.1
   */
  ...
  /**
   * Gets the physics shape of this object. Used to have a reference core to
